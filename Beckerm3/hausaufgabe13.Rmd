...
# ich finde die wide table übersichtlicher, da hier pro Subject "nur" eine Zeile ausgegeben wird. 
...
# ich finde den 2. Plot (jitter) nicht so angenehm, wie das Original. Vor allem der 4. Plot (boxplot) zeigt genauer, wie ähnlich oder nicht ähnlich der Mittelwert der einzelnen Varianten ist.
...
# die Mittelwerte scheinen weiter gestreut zu sein.
...
```
# komische Fehlermeldung...(Warning: Collapsing data to cell means. *IF* the requested effects are a subset of the full design, you must use the "within_full" argument, else results may be inaccurate.)
