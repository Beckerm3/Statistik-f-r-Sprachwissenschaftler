...
# 2014-05-01
# Diese Hausaufgaben müssen nicht unbedingt als Beispiel im Unterricht genutzt werden. ;)
...
dat <- read.table("Beckerm3/body_dim_long.tab",header=TRUE) 
...
frauen <- subset(dat, sex=="f")
  # da ich mich nicht an die Spaltennamen erinnere, lass ich mir die Struktur des Datensatzes "dim" anzeigen...
str(dat)
  # sweet - jetzt weiß ich, dass ich die Frauen unter "sex" und nicht "gender" suchen muss ;) - nun also zurück zu Zeile 74....
...
# zweimal machen und schauen, wie sich die Ergebnisse geändert haben.)
  # Da wir unsre Daten nun im Bezug auf die Randgruppe "Männer" bereinigen werden zwar die oberen Ausreißer entfernt, aber 
  # andererseits verringen wir auch die "durchschnittliche Größe"... Spontan - und ganz naiv würde ich sagen, dass Vergleiche
  # mit anderen Studiengängen nur dort sinnvoll wären, wenn auch die Vergleichsstudiengänge stark weiblich
  # dominiert wären. Sollten wir aber z.B. - ganz sterotypisch - mit einem Maschinenbaustudium vergleichen hätten
  # wir dort weitaus weniger Datensätze (da wahrscheinlich weniger weibliche Studierende) und ein Vergleich wäre eher sinnlos.
...
frauen.studiengang.dichte <- ggplot(data=frauen, aes(x=major)) + geom_density()
...
# HINT: Formel und Beispiel für die Berechnung auf den Folien!
  # Stabw. = Quadratwurzel(Varianz)
klinisch.sd <- sqrt(var(klinisch[, "height"]))
linkk.sd <- sqrt(var(linkk[, "height"]))
speech.sd <- sqrt(var(speech[, "height"]))
